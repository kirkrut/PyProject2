...
for i in range (1, n+1):
    if wks.cell(ans[0], i).value!= '' and wks.cell(ans[1], i).value!= '':
        n1+=1
...
for i in range(1,n+1):
    if wks.cell(ans[0], i).value== '' and i == 1:
        wks.update_cell(ans[0], i, (int(wks.cell(ans[1], i).value)-coef_b)/coef_a)# если не хватает первого элемента x воспользуемся мнк
    if wks.cell(ans[1], i).value== '' and i == 1:
        wks.update_cell(ans[1], i, int(wks.cell(ans[0], i).value) * coef_a + coef_b)# если не хватает первого элемента y воспользуемся мнк
    if wks.cell(ans[0], i).value == '':
        wks.update_cell(ans[0], i, int((int(wks.cell(ans[1], i).value)/int(wks.cell(ans[1], i-1).value)*int(wks.cell(ans[0], i-1).value))))
    if wks.cell(ans[1], i).value == '':
